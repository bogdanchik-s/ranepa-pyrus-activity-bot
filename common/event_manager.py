...
class EventManagerMixin[EventEmitterType: EventEmitter](ABC):
    """
    Менеджер событий
    """
    ...
    def __class_getitem__(cls, typevar_values: type[Any] | tuple[type[Any], ...]) -> type[Self]:
        ...
        for typevar_value in typevar_values:
            if issubclass(typevar_value, EventEmitterType.__bound__ or type(None)):
                cls.__event_emitter__ = typevar_value()
                break
